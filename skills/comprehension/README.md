...
most of. Each skill rests on a specific finding rather than on a metaphor.
    ...
    whiteboard-defense/   pre-shipping check that what the developer thinks the change does matches the diff

All six are stateless: nothing is written outside the code and its history.
...
Thoughtworks Radar entry on codebase cognitive debt; the "whiteboard defense"
benchmark for responsible AI use: you should be able to explain how what you
shipped works and defend its decisions, adversaries, data structures, and
failure modes, without needing line-level recall.
